...
def get_trend_signal():
    ...
    df = pd.read_sql("SELECT * FROM eurusd_prices ORDER BY timestamp DESC LIMIT 20", conn)
    conn.close()
...
def get_cot_bias():
    conn = sqlite3.connect('db/database.db')
    df = pd.read_sql("SELECT * FROM cot_reports ORDER BY report_date DESC LIMIT 2", conn)
    conn.close()
...
def get_sentiment_signal():
    conn = sqlite3.connect('db/database.db')
    df = pd.read_sql("SELECT * FROM sentiment_data ORDER BY timestamp DESC LIMIT 1", conn)
    conn.close()
...
def generate_trade_signal():
    ...
    conn = sqlite3.connect('db/database.db')
    df = pd.read_sql("SELECT * FROM eurusd_prices ORDER BY timestamp DESC LIMIT 1", conn)
    conn.close()
    ...
    if "Uptrend" in trend and "Buying" in bias and "Balanced" in sentiment:
        sl = latest_price - 0.0015  # 15 pips
        tp = latest_price + 0.0030  # 30 pips
        return {
            "signal": "✅ BUY SETUP",
            "entry": latest_price,
            "tp": tp,
        ...
        sl = latest_price + 0.0015
        tp = latest_price - 0.0030
        return {
            "signal": "🔻 SELL SETUP",
            "entry": latest_price,
            "tp": tp,
